...
toc()  #took 4 sec where time.step = NA
...
plotly::ggplotly(
  ggplot() +
    ...
    geom_path(data = res_crw_fitted, aes(lon, lat, group = id, color = id), linewidth = 0.75, alpha = 0.8) +
...
plotly::ggplotly(
  ggplot() +
    # geom_sf(data = brazil, fill = "grey60") +
    geom_point(data = res_crw_fitted, aes(lon, lat, group = id, color = g), linewidth = 0.75, alpha = 0.8) +
...
fit_mp_8hr <- fit_ssm(dat2, vmax = 3, model = "mp", time.step = 8,
                       control = ssm_control(verbose = 1), map = list(psi = factor(NA))
                      )
toc()  #took 6 sec to fit model

print(fit_mp_8hr)


# Viz the filtered outliers (x's), raw observations (blue), and estimated locations (gold), along with associated uncertainty (shading)
...
## Of these 3 different models, the CRW model w/ 8 hr prediction time step seems to give best coarse-scale state estimates
...
write_csv(res_mp_8hr_pred, "Processed_data/SSM_mp8hr_FDN Cmydas tracks.csv")
